...
/**************************************
    ...
    3. 설치:
        ...
        - 설치후 import로 사용할 페이지에 패키지를 포함시킴
        import Vue from 'vue'
        import VueRouter from 'vue-router'
        Vue.use(VueRouter) -> 뷰JS에서 라우터 사용시 등록 필수
    
        (3) 뷰 인스턴스 객체 내부에 옵션으로 반드시 등록 해주어야 사용할 수 있다.
        예) new Vue({
            el:"",
            router, -----> 등록 필수
        })
        참고) router변수와 옵션 routes변수는 다른 이름으로 변경 불가 함

    4. 버전: Vue 2.x버전일 경우 Vue Router 3.x 버전을 사용함

    5. 내장 컴포넌트
        (1) <router-view> : 라우트와 일치하는 컴포넌트를 렌더링함
...
let routes = [
    ...
    // 두번째 루트의 파라미터 버전
    {
        // 파라미터를 받는 같은 path의 루트는 호출과 구분하기 위해 반드시 name속성을 설정해야 한다
        name: "umsik",
        path: "/foods:item",
        // 경로뒤에 콜론을 쓰고 뒤에 파라미터 변수를 써준다
        component: Foods,
    },
];
